...
let isCursorMouseDown = false;
let mouseUpAnimation = false;
document.addEventListener('mousedown', (event) => {
    ...
    isCursorMouseDown = true;
    mouseUpAnimation = false;
});
document.addEventListener('mouseup', (event) => {
    isCursorMouseDown = false;
    mouseUpAnimation = true;
    const cursor = document.getElementById('cursor');
    cursor.style.transform = 'scale(1)';
    cursor.style.transition = 'transform 0.1s';
    setTimeout(() => {
        mouseUpAnimation = false;
    }, 120); // Un poco más que el transition para evitar parpadeos
});
...
document.addEventListener('touchmove', (e) => {
    ...
    if (imageRow) {
        ...
        if (isVerticalCarousel()) {
            imageRow.scrollTop = scrollPositionY + (deltaY *2) + (deltaX *2); // Desplazamiento vertical con horizontal aplicado
        } else {
            imageRow.scrollLeft = scrollPositionX + (deltaX *2) + (deltaY *2); // Desplazamiento horizontal con vertical aplicado
        }
    }
});
...
function smoothTouchScroll() {
    ...
    if (imageRow && (Math.abs(touchVelocityX) > 0.5 || Math.abs(touchVelocityY) > 0.5)) {
        if (isVerticalCarousel()) {
            imageRow.scrollTop += touchVelocityY * 0.1 + touchVelocityX * 0.1; // Desplazamiento vertical con horizontal aplicado
        } else {
            imageRow.scrollLeft += touchVelocityX * 0.1 + touchVelocityY * 0.1; // Desplazamiento horizontal con vertical aplicado
        }
...
function updateCursorState() {
    ...
    // Si no está sobre ningún elemento interactivo ni .force-expand
    else {
        if (isCursorMouseDown) {
            cursor.style.transform = 'scale(1.5)';
            cursor.style.transition = 'transform 0.1s';
        } else if (!mouseUpAnimation) {
            cursor.style.transform = 'scale(1)';
            cursor.style.transition = 'transform 0s';
        }
        // Si mouseUpAnimation está activo, no sobrescribas el estilo, deja que el evento mouseup lo controle
    }
...
// Actualiza el comportamiento de overscroll y overflow en .row
function updateOverscrollAndOverflow() {
    const imageRow = document.querySelector('.row');
    if (!imageRow) return;

    let atStart = false;
    if (isVerticalCarousel()) {
        atStart = imageRow.scrollTop === 0;
    } else {
        atStart = imageRow.scrollLeft === 0;
    }
...
// Ejecuta al cargar la página y al hacer scroll en .row
window.addEventListener('load', updateOverscrollAndOverflow);
window.addEventListener('resize', updateOverscrollAndOverflow);
if (imageRow) {
    imageRow.addEventListener('scroll', updateOverscrollAndOverflow);
}
...
function updateSignScrollVisibility() {
    if (!imageRow || !signScroll || !signScrollmedia) return;

    if (isVerticalCarousel()) {
        // Scroll vertical disponible
        if (imageRow.scrollHeight > imageRow.clientHeight) {
            signScrollmedia.style.display = 'flex';
        } else {
            signScrollmedia.style.display = 'none';
        }
        // Oculta el horizontal siempre
        signScroll.style.display = 'none';
    } else {
        // Scroll horizontal disponible
        if (imageRow.scrollWidth > imageRow.clientWidth) {
            signScroll.style.display = 'flex';
        } else {
            signScroll.style.display = 'none';
        }
        // Oculta el vertical siempre
        signScrollmedia.style.display = 'none';
    }
}

// Ejecuta al cargar, redimensionar y al hacer scroll
window.addEventListener('load', updateSignScrollVisibility);
window.addEventListener('resize', updateSignScrollVisibility);
if (imageRow) {
    imageRow.addEventListener('scroll', updateSignScrollVisibility);
}